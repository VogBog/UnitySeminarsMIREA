...
{
    public class DefaultGridMap : IGridMap
    {
        ...
        public DefaultGridMap(GridMap gridMap)
        {
            _gridMap = gridMap;
            _bounds = gridMap.Bounds;
            _cellsPerUnit = gridMap.CellsPerUnit;
            _chunkSize = gridMap.ChunkSize;
            
            CreateGrid();
            _gridMap.StartCoroutine(LifetimeRoutine());
        }
        ...
        {
            var rule = new RuleData(_gridMap, new(x, y), _map[x, y], c);
        ...
        private void SetCells(SetCellsRect rect)
        {
            ...
            for (int x = minX; x < maxX; x++)
            {
                for (int y = minY; y < maxY; y++)
                {
                    ...
                    var rule = new RuleData(_gridMap, new(x, y), _map[x, y], rect.Value);
                    int value = SetCellRule(rule, out bool changed);
        ...
        public void SetCellsAsync(SetCellsRect rect)
        {
            if (rect.Rect.width < _chunkSize && rect.Rect.height < _chunkSize)
            {
                ...
                if (!_processingJobs)
                {
                    ...
                    _gridMap.StartCoroutine(ProcessJobs());
                }
        ...
        {
            var list = new List<SetCellsRect>();
            var aabb = IGridMap.CalculateAABB(lb, lt, rb, rt);
            ...
            int centerX, int centerY, int maxDist, byte value, Predicate<byte> predicate)
        {
            ...
            var aabb = IGridMap.CalculateAABBInt(paintedAreas);
        ...
        {
            _gridMap.StartCoroutine(DelayedCallRoutine(call));
        }
